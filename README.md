...
├── visitas.html          # Visitas técnicas (qualquer um cria, autor/admin edita)
...
└── storage.rules         # Regras de segurança dos arquivos/laudos (você vai colar no console do Firebase)
```
...
- **Relatórios**: cada técnico monta o relatório diário de atividades como uma pequena tabela (igual ao modelo em papel/planilha usado pela equipe): para cada linha, informa categoria, atividade, quantidade/área e status (Concluído, Em andamento ou Pendente), podendo adicionar quantas linhas quiser com o botão "+ Adicionar atividade". Um campo de resumo no final é opcional. Um técnico vê só o próprio histórico; um admin vê os relatórios de todo o time nesta mesma tela, com um filtro por técnico, e pode editar ou excluir qualquer um deles.
- **Visitas Técnicas**: registro de visitas em campo (número, título, recurso responsável, cidade, data, área, tipo de atendimento, status e empresa responsável — "Equipe Interna" ou "FindUp"). Qualquer pessoa logada pode registrar uma visita; editar uma visita já existente só é permitido para quem a criou ou para um admin; excluir é só admin. O Dashboard mostra um gráfico comparando o volume de visitas por empresa responsável.
  - Para marcar uma visita como **Realizada**, é obrigatório anexar o **laudo em PDF ou DOCX** (o sistema bloqueia o salvamento sem o arquivo). O laudo fica guardado no Firebase Storage e aparece como um link para download na coluna "Laudo" da tabela.
  - Como o site não tem um servidor próprio para disparar e-mails sozinho, os avisos por e-mail funcionam assim: ao registrar uma nova visita, e ao marcá-la como Realizada com o laudo anexado, aparece uma confirmação perguntando se você quer **abrir um e-mail já pronto** (destinatários, assunto e corpo preenchidos — incluindo o link do laudo, no caso da visita realizada) no seu programa de e-mail padrão. Você só precisa clicar em "Enviar" nele. Isso acontece na hora em que a pessoa salva a visita pelo site — não é um envio automático "no fundo", então se ninguém abrir/salvar aquela visita pelo site, o e-mail não é dis­parado sozinho.
...
- **Usuários** (somente admin): não existe cadastro público — só um admin cria novas contas por aqui (nome, e-mail, senha temporária e papel), além de promover/rebaixar técnicos e admins. Um admin também pode excluir um usuário técnico: isso remove o acesso e o perfil dele no site, mas não apaga a conta de login (Firebase Authentication) — para liberar totalmente o e-mail, remova-o também em Authentication → Users no Console do Firebase. Não é possível excluir um admin por aqui (é preciso rebaixá-lo a técnico primeiro).
...
## Passo 4 — Ativar o Storage (para os laudos das visitas)

O módulo de **Visitas Técnicas** guarda o laudo (PDF ou DOCX) anexado a cada visita marcada como "Realizada" no **Firebase Storage** — um espaço de arquivos separado do Firestore (que só guarda texto/números).

1. No menu lateral, vá em **Compilar (Build) → Storage**.
2. Clique em **Vamos começar (Get started)** e siga o assistente (pode manter as opções padrão de local/modo).
3. Depois de criado, vá na aba **Regras (Rules)** e substitua todo o conteúdo pelo texto do arquivo **`storage.rules`** (está nesta pasta do projeto, ao lado do `firestore.rules`). Clique em **Publicar (Publish)**.

Essas regras garantem que só quem está logado consegue enviar ou baixar um laudo, e que só aceitam arquivo PDF ou DOCX de até 15 MB.

## Passo 5 — Pegar as credenciais do app web
...
## Passo 6 — Subir o projeto para o GitHub
...
## Passo 8 — Liberar o domínio do GitHub Pages no Firebase
...
## Passo 9 — Criar o primeiro admin (manualmente, só uma vez)
...
## Passo 10 — Cadastrando o restante do time
...
Depois acesse `http://localhost:8000` no navegador. Lembre-se de adicionar `localhost` como domínio autorizado no Firebase (Passo 8) se quiser testar o login localmente.
